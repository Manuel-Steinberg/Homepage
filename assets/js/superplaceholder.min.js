(function(){var test=document.createElement('input');var isPlaceHolderSupported=('placeholder' in test);function extend(obj1,obj2){var obj={};for(var key in obj1){obj[key]=obj2[key]===undefined?obj1[key]:obj2[key]}
    return obj}
    ...
    PlaceHolder.prototype.begin=function(){var self=this,temp,randomIndex;self.originalPlaceholder=self.el.getAttribute('placeholder');if(self.options.shuffle){for(var i=self.texts.length;i--;){randomIndex=~~(Math.random()*i);temp=self.texts[randomIndex];self.texts[randomIndex]=self.texts[i];self.texts[i]=temp}}
        if(self.options.startOnFocus){self.el.addEventListener('focus',function(){self.processText(0)});self.el.addEventListener('blur',function(){self.cleanUp()})}
        ...
        this.el.setAttribute('placeholder',this.originalPlaceholder);this.timeouts.length=0};PlaceHolder.prototype.typeString=function(str,callback){var self=this,timeout;if(!str){return!1}
